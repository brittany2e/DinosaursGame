...
namespace DinosaursGame
{
    class MainScreen : Screen
    {
        ...
        private void SetUpPlants(ContentManager content)
        {
            ...
            for (int i = 5; i < numberOfPlants; i++)
            {
                Random rand = new Random();
                int x = rand.Next() % (data.screenWidth - plants[i].Texture.Width);
                int y = rand.Next() % (data.screenHeight - plants[i].Texture.Height);
        ...
        private void SetUpEnemies(ContentManager content)
        {
            ...
            for (int i = 0; i < numberOfEnemies; i++)
            {
                ...
                Random rand = new Random();
                int x = rand.Next() % (data.screenWidth - - enemies[i].Texture.Width);
                int y = rand.Next() % (data.screenHeight - enemies[i].Texture.Height);
